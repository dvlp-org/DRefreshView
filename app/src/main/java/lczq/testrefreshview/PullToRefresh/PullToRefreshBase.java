...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
     ...
     * @since 2018-3-15
     */
    public interface OnRefreshAnimationListener<V extends View> {
         ...
         * @param refreshView 刷新的View
         */
        void onPullDownToRefreshStart(final PullToRefreshBase<V> refreshView);
         ...
         * @param refreshView 刷新的View
         */
        void onPullDownToRefreshOver(final PullToRefreshBase<V> refreshView);
    }
    ...
    /**下拉刷新和加载更多的动画监听器 */
    private OnRefreshAnimationListener<T> mRefreshAnimationListener;
    /**下拉刷新的布局 */
...
//        LogUtil.printLog("i", "PullToRefreshBase headerHeight == " + headerHeight);
    ...
    @Override
    public void setOnRefreshAnimationListener(OnRefreshAnimationListener<T> refreshAnimationListener) {
        mRefreshAnimationListener=refreshAnimationListener;
    }

    @Override
    public void onPullDownRefreshComplete() {
        if (isPullRefreshing()) {
            mHeaderLayout.onRefreshingEnd();
    ...
    protected void pullHeaderLayout(float delta) {
        ...
        Log.i("lius", "下拉动画开始移动距离"+delta+"========");
        if (null !=mRefreshAnimationListener) {
            mRefreshAnimationListener.onPullDownToRefreshStart(PullToRefreshBase.this);
        }
        // 向下滑动布局
    ...
    protected void resetHeaderLayout() {
        ...
        if (refreshing && scrollY <= mHeaderHeight) {
            smoothScrollTo(0);
            if (null !=mRefreshAnimationListener) {
                mRefreshAnimationListener.onPullDownToRefreshOver(PullToRefreshBase.this);
            }
            return;
        }
